...
export class ProductDashComponent implements OnInit{
  ...
  deleteProduct(id: any, event?: DragEvent): void {
    ...
    if (confirm("Êtes-vous sûr de vouloir supprimer cette Product ?")) {
      this.productService.deleteProduct(id).subscribe({
        next: () => {
          window.alert("Product supprimée avec succès !");
  ...
  saveNewProduct() {
    ...
    this.productService.createProduct(Product).subscribe({
      next: (newProduct) => {
        ...
        window.alert("Product ajoutée avec succès !");
        window.location.reload();
        this.closeNewProductForm(); // Optionally close the form
      },
      error: (error) => {
        console.error(error);
        window.alert("Une erreur s'est produite lors de l'ajout de la Product. Veuillez réessayer plus tard.");
      }
    });
  }
    ...
    this.productService.updateProduct(this.selectedProduct.id, updatedProduct).subscribe({
      ...
      error: (error) => {
        ...
        if (error.status === 200) {
          ...
          window.alert("An error occurred while updating the product. Please try again later.");
        }
      }
    });
  }
